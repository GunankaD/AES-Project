...
int main() {
    ...
    cout << "AES-128 Algorithm. Choose the operation you want to perform.\n";
    cout << "1. Encrypt\n";
    cout << "2. Decrypt\n";
    ...
    if (choice == 1) {
        cout << "Enter plaintext (any length): ";
        getline(cin, input);
        
        cout << "Ciphertext: ";
        string encrypted = encryptAES(input, keyMatrix, s_box);
        cout << encrypted << endl;
        
    } else if (choice == 2) {
        cout << "Enter ciphertext (hex string, multiple of 32 characters): ";
        getline(cin, input);
        if (input.length() % 32 != 0) {
            cout << "Error: Invalid ciphertext length\n";
            return 1;
        }
        
        cout << "Plaintext: ";
        string decrypted = decryptAES(input, keyMatrix, s_box, inv_s_box);
        cout << decrypted << endl;
