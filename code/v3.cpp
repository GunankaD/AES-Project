...
int main() {
    ...
    if (plaintext.size() != 16 || key.size() != 16) {
        cerr << "Error: Plaintext and key must each be exactly 16 characters." << endl;
        return 1;
    }

    Matrix plaintextMatrix = convertToHexMatrix(plaintext);
    Matrix keyMatrix = convertToHexMatrix(key);

    cout << "Encrypting data..." << endl;
    aesEncrypt(plaintextMatrix, keyMatrix, s_box);
