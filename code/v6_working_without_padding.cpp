...
int main() {
    ...
    if (choice == 1) {
        ...
        getline(cin, input);
        Matrix plaintextMatrix = convertToHexMatrix(input);

        cout << "Ciphertext: ";
        aesEncrypt(plaintextMatrix, keyMatrix, s_box, encryptedMatrix);
    } else if (choice == 2) {
        cout << "Enter ciphertext (32 hex characters): ";
        getline(cin, input);
        Matrix ciphertextMatrix = hexStringToMatrix(input);

        cout << "Plaintext: ";
        aesDecrypt(ciphertextMatrix, keyMatrix, s_box, inv_s_box, decryptedMatrix);
    } else {
        cout << "Invalid choice!\n";
    }
